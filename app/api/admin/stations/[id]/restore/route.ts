...
export async function POST(
  ...
  try {
    await hardwareAdminService.restoreHardware(auth.auth.userId, id)
    ...
    console.error('[Admin] station restore:', error)
    return NextResponse.json({ success: false, error: 'Failed to restore hardware' }, { status: 500 })
  }
}
