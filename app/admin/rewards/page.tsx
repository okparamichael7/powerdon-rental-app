...
import { mockRewards, mockCampaigns } from "@/lib/mock-data"
...
export default function RewardsPage() {
  ...
  const filteredRewards = mockRewards.filter((reward) => {
    const matchesSearch = 
      reward.code.toLowerCase().includes(searchQuery.toLowerCase()) ||
      reward.userEmail.toLowerCase().includes(searchQuery.toLowerCase())
    const matchesStatus = statusFilter === "all" || reward.status === statusFilter
    const matchesCampaign = campaignFilter === "all" || reward.campaignId === campaignFilter
  ...
  const stats = {
    ...
    issued: mockRewards.filter(r => r.status === "issued").length,
    redeemed: mockRewards.filter(r => r.status === "redeemed").length,
    expired: mockRewards.filter(r => r.status === "expired").length,
    totalValue: mockRewards.reduce((sum, r) => sum + r.value, 0),
    redeemedValue: mockRewards.filter(r => r.status === "redeemed").reduce((sum, r) => sum + r.value, 0),
  }
        ...
        <Table>
          ...
          <TableBody>
            {filteredRewards.map((reward) => {
              const campaign = mockCampaigns.find(c => c.id === reward.campaignId)
              return (
                <TableRow key={reward.id}>
                  ...
                  <TableCell className="text-muted-foreground">{reward.issuedAt}</TableCell>
                  <TableCell className="text-muted-foreground">{reward.expiresAt}</TableCell>
